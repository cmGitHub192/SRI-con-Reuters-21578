...
### Limpieza de texto
def clean_text(text):
    with open(os.path.join(os.getcwd(), 'Proyecto_BIM_I', 'reuters', 'stopwords.txt'), 'r', encoding='utf-8') as file:
...
def search_jaccard(query, inverted_index):
    ...
    ### Carga de documentos
    CORPUS_DIR = os.path.join(os.getcwd(), 'Proyecto_BIM_I', 'reuters','training')
    documents = {}
...
def results(query, tv, tr):
    if tv == "0":
        inverted_index_loaded = load_inverted_index_from_txt(os.path.join(os.getcwd(), 'Proyecto_BIM_I', 'results', 'inverted_index_tf_idf.txt'))
        if tr == "0":
        ...
        inverted_index_loaded = load_inverted_index_from_txt(os.path.join(os.getcwd(), 'Proyecto_BIM_I', 'results', 'inverted_index_bow.txt'))
        if tr == "0":
            results = search_jaccard(query, inverted_index_loaded)
        else:
            results = search_jaccard(query, inverted_index_loaded)
